...
import {
    ...
    setDefault,
    updateOption,
} from "./ManageLOLServerFilter.server";
import { WithId } from "@/utils/types";
import { useEffect, useState } from "react";
import { toast } from "react-toastify";
import { IDefaultFilter } from "@/models/defaultFiltersModel";

const ManageLOLServerFilter = (props: ManageLanguageFilterProps) => {
    const [defaultLang, setDefaultLang] = useState<DropDownItem | string>();
    const langs = props.items.map((x) => ({ label: x.name, value: x._id }));
    useEffect(() => {
    ...
    return (
        <div>
            <h2>Manage Server Filter Settings: </h2>
...
export default ManageLOLServerFilter;
